...
df_commodity = df_commodity[['분류','ff/aum','return_1w','AUM']]

# 고유한 대분류별 색상 매핑
categories = df_commodity['분류'].unique()
colors = {
    "Diversified": "#005BAC",
    "Precious_metals": "#0097E0",
    "Energy&Fuel": "#FABF00",
    "Industrial_metals" : "#D3D3D3",
    "Agriculture" : "#00AA4C"
}
...
for category in categories:
    df_filtered = df_commodity[df_commodity['분류'] == category]
...
# AUM 기준 내림차순 정렬
df_commodity = df_commodity.sort_values(by='return_1w', ascending=False)
...
# AUM 기준 내림차순 정렬
df_commodity_ff_aum = df_commodity.sort_values(by='ff/aum', ascending=False)

# 가로형 막대그래프 생성
fig2 = go.Figure()
for category in colors.keys():
    df_filtered_ff_aum = df_commodity_ff_aum[df_commodity_ff_aum['분류'] == category]
    fig2.add_trace(go.Bar(
